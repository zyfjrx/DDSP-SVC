...
class EnhancedWebUI:
    ...
    def get_speaker_folders(self):
        """获取process_data/raw目录下的说话人文件夹列表"""
        raw_dir = "process_data/raw"
        speakers = []
    ...
    def get_speaker_input_path(self, speaker_name):
        """根据说话人名称获取对应的输入路径"""
        if speaker_name:
    ...
    def create_msst_tab(self):
        ...
        with gr.Row():
            with gr.Column():
                ...
                <div style="text-align: center; margin-bottom: 20px;">
                    <h2 style="color: #2196F3; margin-bottom: 10px;">🎼 MSST 音频分离工具</h2>
                    <p style="color: #666; font-size: 14px;">使用先进的MSST模型进行高质量音频源分离，支持人声、乐器等多种分离模式</p>
                </div>
                """)
        ...
        with gr.Row(equal_height=True):
            ...
            with gr.Column(scale=2):
                with gr.Group():
                    gr.HTML('<h3 style="margin: 10px 0; color: #1976D2;">📁 输入配置</h3>')
                    
                    msst_input_type = gr.Radio(
                        choices=["单个文件", "文件夹批处理"],
                        value="单个文件",
                        label="输入模式",
                        info="选择处理单个文件还是批量处理文件夹"
                    )
                    ...
                    with gr.Group(visible=False) as folder_input_group:
                        with gr.Row():
                            msst_speaker_dropdown = gr.Dropdown(
                                choices=self.get_speaker_folders(),
                                label="👤 选择说话人",
                                info="自动识别process_data/raw目录下的说话人",
                                interactive=True,
                                scale=3
                            )
                        ...
                        msst_input_folder = gr.Textbox(
                            label="📂 输入文件夹路径",
                            placeholder="选择说话人后自动填充路径",
                            value="process_data/raw",
                            interactive=True
                        )
                    
                    msst_output_folder = gr.Textbox(
                        label="📤 输出文件夹",
                        placeholder="根据模型和说话人自动设置",
                        value="process_data/msst",
                        interactive=True,
                        info="💡 输出路径会根据模型类型自动调整"
                    )
            
            # 右侧：模型配置
            with gr.Column(scale=2):
                with gr.Group():
                    gr.HTML('<h3 style="margin: 10px 0; color: #1976D2;">🤖 模型配置</h3>')
                    ...
                    with gr.Row():
                        ...
                        msst_refresh_models_btn = gr.Button(
                            "🔄 刷新", 
                            size="sm",
                            scale=1,
                            variant="secondary"
                    ...
                    # 隐藏的路径变量
                    msst_model_path = gr.Textbox(visible=False)
                    msst_config_path = gr.Textbox(visible=False)
                    
                    # 分离目标设置
                    with gr.Group():
                        gr.HTML('<h4 style="margin: 5px 0; color: #666;">🎯 分离目标</h4>')
        ...
        with gr.Accordion("⚙️ 高级参数设置", open=False):
            with gr.Row():
                with gr.Column():
                    gr.HTML('<h4 style="color: #666; margin-bottom: 10px;">🔧 分离参数</h4>')
                ...
                with gr.Column():
                    gr.HTML('<h4 style="color: #666; margin-bottom: 10px;">📊 音频质量参数</h4>')
        ...
        with gr.Row():
            ...
            with gr.Column(scale=2):
                with gr.Row():
                    msst_start_btn = gr.Button(
                        "🚀 开始分离", 
                        variant="primary", 
                        size="lg",
                        scale=2
                    )
                    msst_stop_btn = gr.Button(
                        "⏹️ 停止分离", 
                        variant="stop", 
                        size="lg",
                        scale=1
                    )
                    msst_refresh_status_btn = gr.Button(
                        "🔄 刷新状态", 
                        variant="secondary", 
                        size="lg",
                        scale=1
        ...
        with gr.Row():
            ...
            # 输出音频预览（仅单文件模式）
            with gr.Column(visible=True) as output_audio_group:
                gr.HTML('<h4 style="color: #666; margin-bottom: 10px;">🎵 分离结果预览</h4>')
        ...
        # 模型选择变化时更新文件路径和配置信息
        def update_model_paths_and_config(model_type, model_name, speaker_name=""):
            ...
            return "", "", gr.Radio(choices=[], visible=False), "请先选择模型", "process_data/msst"
        ...
        # 当模型和说话人都改变时更新输出文件夹
        def update_output_on_change(model_name, speaker_name):
            """当模型或说话人改变时更新输出文件夹"""
            return self.get_output_folder_by_model(model_name, speaker_name)
        
        # 添加说话人选择相关的事件绑定
        def update_input_folder_from_speaker(speaker_name):
            """根据选择的说话人更新输入文件夹路径"""
            if speaker_name:
                return f"process_data/raw/{speaker_name}"
            return "process_data/raw"
        ...
        def refresh_speaker_list():
            """刷新说话人列表"""
            speakers = self.get_speaker_folders()
            if speakers:
        ...
        # 绑定说话人选择事件
        msst_speaker_dropdown.change(
            fn=update_input_folder_from_speaker,
            inputs=[msst_speaker_dropdown],
            outputs=[msst_input_folder]
        ).then(
        ...
        # 绑定刷新说话人列表事件
        msst_refresh_speakers_btn.click(
            fn=refresh_speaker_list,
            outputs=[msst_speaker_dropdown]
        )
