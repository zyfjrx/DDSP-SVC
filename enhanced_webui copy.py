...
class EnhancedWebUI:
    ...
    def create_preprocessing_tab(self):
        ...
        organize_btn.click(
            ...
            outputs=[preprocess_status, data_summary, preprocess_log]
        )
        
        preprocess_btn.click(
            fn=self.start_preprocessing_with_progress,
            outputs=[preprocess_status, preprocess_log, preprocess_progress],
            show_progress=True
        )
    ...
    def organize_data(self, source_data_dir: str, model_type: str, train_ratio: float, multi_train_ratio: float, 
        ...
        try:
            ...
            return "✅ 数据组织完成", summary, log_content
            
        except Exception as e:
            print(f"[数据组织] ❌ 数据组织失败: {str(e)}")
            return f"❌ 数据组织失败: {str(e)}", "", f"❌ 错误详情: {str(e)}"
    ...
    def start_preprocessing_with_progress(self):
        """带进度条的预处理方法"""
        print("[预处理] 🚀 预处理开始")
        yield "🚀 预处理开始", "🚀 正在启动预处理进程...", 5
        
        if self.current_process and self.current_process.poll() is None:
            print("[预处理] ❌ 已有进程在运行")
            yield "❌ 已有进程在运行", "❌ 已有进程在运行，请先停止当前进程", 0
            return
        ...
        try:
            ...
            print("[预处理] ⚙️ 预处理运行中")
            yield "⚙️ 预处理运行中", "⚙️ 预处理进程已启动，正在处理数据...", progress
                ...
                try:
                    ...
                    if line:
                        ...
                        if progress_match:
                            ...
                            yield f"处理中 ({actual_progress}%)", f"正在处理...\n\n{''.join(output_lines[-8:])}", progress
                except Exception as e:
            ...
            if return_code == 0:
                ...
                print("[预处理] ✅ 预处理成功完成!")
                yield "✅ 预处理完成", final_log, 100
            else:
                error_log = f"❌ 预处理失败 (退出代码: {return_code})\n\n错误输出:\n{''.join(output_lines[-15:])}"
                print(f"[预处理] ❌ 预处理失败 (退出代码: {return_code})")
                yield "❌ 预处理失败", error_log, progress
        
        except Exception as e:
            error_msg = f"❌ 预处理启动失败: {str(e)}"
            print(f"[预处理] {error_msg}")
            yield "❌ 启动失败", error_msg, 0
